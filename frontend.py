...
if 'roles_cache' not in st.session_state:
    st.session_state.roles_cache = None
if 'skills_cache' not in st.session_state:
    st.session_state.skills_cache = None
if 'providers_cache' not in st.session_state:
    st.session_state.providers_cache = None
...
def load_roles():
    if st.session_state.roles_cache is None:
        st.session_state.roles_cache = fetch_json("/api/roles")
    return st.session_state.roles_cache

def load_skills():
    if st.session_state.skills_cache is None:
        st.session_state.skills_cache = fetch_json("/api/skills")
    return st.session_state.skills_cache

def load_providers():
    if st.session_state.providers_cache is None:
        st.session_state.providers_cache = fetch_json("/api/providers")
    return st.session_state.providers_cache
...
roles_data = load_roles() or []
skills_data = load_skills() or []
providers_data = load_providers() or []

role_options = {}
for r in roles_data:
    label = f"{r.get('role_name', '')} ({r.get('domain', '')})"
    role_options[r['role_id']] = {"label": label, "name": r.get('role_name', ''), "domain": r.get('domain', ''), "level": r.get('level', ''), "description": r.get('description', ''), "required_skills": r.get('required_skills', ''), "optional_skills": r.get('optional_skills', ''), "tools": r.get('tools', ''), "average_learning_months": r.get('average_learning_months', ''), "priority_score": r.get('priority_score', ''), "market_demand_score": r.get('market_demand_score', '')}

all_api_skills = sorted(set(s.get('normalized_skill_name', '') for s in skills_data if s.get('normalized_skill_name')))
...
def analyze_resume(text):
    ...
    if len(numbers) >= 3:
        score += 3
        feedback.append(("success", "Quantifiable achievements found"))
    else:
...
with tab1:
    ...
    with col_input:
        ...
        role_display_map = {v["label"]: k for k, v in role_options.items()}
        selected_role_label = st.selectbox("Select your dream job:", list(role_display_map.keys()) if role_display_map else list(SKILLS_DB_FALLBACK.keys()), label_visibility="collapsed")
        selected_role_id = role_display_map.get(selected_role_label)
        selected_role_name = role_options[selected_role_id]["name"] if selected_role_id and selected_role_id in role_options else selected_role_label

        if selected_role_id and selected_role_id in role_options:
            r = role_options[selected_role_id]
            with st.expander(" Role Details", expanded=False):
                req_skills = r.get('required_skills', '').replace(';', ', ')
                opt_skills = r.get('optional_skills', '').replace(';', ', ')
                st.markdown(f"""
| Metric | Value |
|--------|-------|
| **Domain** | {r.get('domain', 'N/A')} |
| **Level** | {r.get('level', 'N/A').title()} |
| **Learning Months** | {r.get('average_learning_months', 'N/A')} |
| **Priority Score** | {r.get('priority_score', 'N/A')}/100 |
| **Market Demand** | {r.get('market_demand_score', 'N/A')}/100 |
| **Required Skills** | {req_skills} |
| **Optional Skills** | {opt_skills} |
| **Tools** | {r.get('tools', 'N/A').replace(';', ', ')} |
""")

    with col_skills:
        st.markdown("###  Your Skills")
        available_skills = all_api_skills if all_api_skills else ALL_SKILLS_FALLBACK
        selected_skills = st.multiselect("Select skills you have:", available_skills, default=[available_skills[0]] if available_skills else [], label_visibility="collapsed")
    ...
    if st.button(" Generate Comprehensive Report", use_container_width=True, type="primary"):
        if not uploaded_resume:
            st.warning(" Please upload your resume for full analysis")
            st.stop()
        ...
        with st.spinner(" Analyzing your profile..."):
            ...
            flask_data = {}
            try:
                response = requests.post(f"{API_BASE}/api/analyze", json=flask_payload, timeout=5)
            ...
            with result_col2:
                ...
                if flask_data.get('ai_role'):
                    st.info(f" **Phase 4 Custom AI Classification:** Your resume matches: **{flask_data['ai_role']}**")
            ...
            with result_col3:
                ...
                if selected_role_id and selected_role_id in role_options:
                    req_skills_str = role_options[selected_role_id].get('required_skills', '')
                    required_skills = [s.strip() for s in req_skills_str.split(';') if s.strip()] if req_skills_str else []
                else:
                    required_skills = SKILLS_DB_FALLBACK.get(selected_role_name, {}).get('required', [])
                ...
                st.markdown(f"**Match: {round(match_pct, 0)}%** for {selected_role_name}")
            ...
            if flask_data.get('missing_skills'):
                ...
                with gap_col2:
                    ...
                    for item in flask_data.get('remedial_roadmap', []):
                        ...
                        if not video_link:
                            video_link = f"https://www.youtube.com/results?search_query={item.get('skill', '').replace(' ', '+')}+course"
...
with tab2:
    ...
    if roles_data:
        ...
        domain_groups = df_roles.groupby('domain')

        market_col1, market_col2, market_col3 = st.columns(3)
        with market_col1:
            st.markdown("####  Roles by Domain")
            for domain, group in domain_groups:
                st.markdown(f"**{domain}** ({len(group)} roles)")
                for _, r in group.iterrows():
                    st.markdown(f"- {r['role_name']} ({r.get('level', 'N/A')})")

        with market_col2:
            st.markdown("####  Top Roles by Demand")
            top_demand = df_roles.nlargest(5, 'market_demand_score')[['role_name', 'market_demand_score', 'domain']]
            for _, r in top_demand.iterrows():
                st.markdown(f"**{r['role_name']}** - Demand: {r['market_demand_score']}/100")

        with market_col3:
            st.markdown("####  Fastest Learning Paths")
            fastest = df_roles.nsmallest(5, 'average_learning_months')[['role_name', 'average_learning_months', 'level']]
            for _, r in fastest.iterrows():
                st.markdown(f"**{r['role_name']}** - {r['average_learning_months']} months ({r.get('level', 'N/A')})")

        st.markdown("---")
        st.markdown("###  All Roles Comparison")
        display_cols = ['role_name', 'domain', 'level', 'priority_score', 'market_demand_score', 'average_learning_months']
        st.dataframe(df_roles[display_cols].rename(columns={
            'role_name': 'Role', 'domain': 'Domain', 'level': 'Level',
            'priority_score': 'Priority', 'market_demand_score': 'Demand',
            'average_learning_months': 'Months'
        }), use_container_width=True)

        if providers_data:
            st.markdown("###  Course Providers Overview")
            df_providers = pd.DataFrame(providers_data)
            st.dataframe(df_providers[['provider_name', 'provider_type', 'trust_score', 'priority_level', 'free_courses_available', 'paid_courses_available', 'certificate_supported', 'country']].rename(columns={
                'provider_name': 'Provider', 'provider_type': 'Type',
                'trust_score': 'Trust Score', 'priority_level': 'Priority',
                'free_courses_available': 'Free', 'paid_courses_available': 'Paid',
                'certificate_supported': 'Certificates', 'country': 'Country'
            }), use_container_width=True)
    else:
        st.info(" Using fallback market data (backend offline)")
        market_col1, market_col2, market_col3 = st.columns(3)
        with market_col1:
        ...
        with market_col3:
            ...
            for skill in ["LLM & Generative AI", "Cloud (AWS/GCP)", "MLOps", "Real-time Analytics", "Data Visualization", "Python + SQL"]:
                st.markdown(f" **{skill}**")

        st.markdown("###  Role Comparison")
        st.dataframe(pd.DataFrame({'Role': list(SKILLS_DB_FALLBACK.keys()), 'Demand': [SKILLS_DB_FALLBACK[r]['demand'] for r in SKILLS_DB_FALLBACK], 'Salary': [SKILLS_DB_FALLBACK[r]['salary_range'] for r in SKILLS_DB_FALLBACK], 'Growth': [SKILLS_DB_FALLBACK[r]['growth'] for r in SKILLS_DB_FALLBACK]}), use_container_width=True)

with tab3:
    st.markdown("###  Personalized Learning Path\nBased on your profile and target role, here's your curated learning roadmap:")

    if selected_role_id:
        recs = fetch_json(f"/api/recommendations?role_id={selected_role_id}&limit=20")
    else:
        recs = None

    if recs:
        priority_1 = [r for r in recs if r.get('required_or_optional') == 'recommended']
        priority_2 = [r for r in recs if r.get('required_or_optional') == 'foundation']
        others = [r for r in recs if r not in priority_1 and r not in priority_2]
        ...
        with learning_col2:
            st.markdown("####  Foundation Courses")
            if priority_2:
        ...
        if selected_role_id and selected_role_id in role_options:
            r = role_options[selected_role_id]
            st.markdown("---")
            st.markdown("###  Role-Specific Learning Plan")
            st.markdown(f"""
| Phase | Duration | Focus |
|-------|----------|-------|
| Foundation | 2-3 months | Core skills: {r.get('required_skills', 'N/A')[:80]}... |
| Intermediate | 3-4 months | Advanced concepts & {r.get('optional_skills', 'project')[:60]}... |
...
""")
        ...
        p1 = priority_1.get(selected_role_name, [])
        p2 = priority_2.get(selected_role_name, [])
        learning_col1, learning_col2 = st.columns(2)
        with learning_col1:
    ...
    if providers_data:
        st.markdown("###  Top-Rated Course Providers")
        top_providers = sorted(providers_data, key=lambda p: p.get('trust_score', 0), reverse=True)[:6]
        cols = st.columns(3)
...
st.markdown("---\n<div style='text-align: center; color: #666; padding: 20px;'><p>SkillGap AI Pro (c) 2026 | Powered by Machine Learning</p><p style='font-size: 0.8rem;'>Model Accuracy: 75%+ | Updated for 2026 Market | Course Engine: 68K+ courses, 34 providers</p></div>", unsafe_allow_html=True)
